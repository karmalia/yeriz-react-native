  ...
  return (
    <TouchableOpacity
      onPress={() => {
        changeOrdering(
          orderings.data.find(
            (ordering) => ordering.name === "En Çok Değerlendirilenler"
          )!
        );
...
const CompaniesMostRated = () => {
  ...
  const { data, isLoading, isError } = useSearchedCompanies(
    ...
    "En Çok Değerlendirilernler"
  );
  console.log("CompaniesMostRated Rendered");
  return (
    <>
...
export default CompaniesMostRated;
