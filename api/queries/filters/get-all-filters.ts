...
export interface GetAllFiltersResponse {
  byCuisineCategory: Filter;
  byPaymentType: Filter;
  byPriceRange: Filter;
  byPoint: Filter;
}
...
const fetchFilters = async (): Promise<GetAllFiltersResponse> => {
  const response = await api.get("/Companies/GetAllFilters");
